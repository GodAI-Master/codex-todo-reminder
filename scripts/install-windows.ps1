...
$action = New-ScheduledTaskAction -Execute (Join-Path $PSHOME "powershell.exe") -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$supervisor`" -DebugPort $DebugPort" -WorkingDirectory $projectRoot
...
try {
  Register-ScheduledTask -TaskName $taskName -Action $action -Trigger @($logonTrigger, $watchdogTrigger) -Settings $settings -Description "Keeps Codex Todo Reminder running" -Force | Out-Null
} catch {
  $runCommand = "powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$supervisor`" -DebugPort $DebugPort"
